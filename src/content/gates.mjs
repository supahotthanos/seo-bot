...
export function scoreContent(draft = '', brief = {}, { priorTexts = [], reviewers = null } = {}) {
  ...
  const unsourced = numbers(text).filter((n) => !allowed.has(n) && !yearOk(n) && (Number(n) > 9 || n.includes('.'))); // also catch fabricated decimals < 10 (e.g. "3.2 sessions", "4.9 stars")
  hard['no-fabrication'] = unsourced.length === 0;
  // originality vs prior/competitor texts
  const sh = shingles(text);
  // A draft with fewer than 8 words produces an empty shingle set → jaccard always 0 → passes trivially.
  // Fail closed: a too-short draft cannot be evaluated and must not be published.
  const maxSim = priorTexts.filter((t) => typeof t === 'string').reduce((m, t) => Math.max(m, jaccard(sh, shingles(t))), 0);
  hard['originality-dedup'] = sh.size > 0 && maxSim < 0.86;
  // Local-VALUE hard gate (programmatic geo pages only — brief.local). A city TOKEN is not local
  // value: June-2026 hit swapped-city templates; survivors had real per-city facts. Require >=2 of
  // {price near the city, named credentialed provider, neighborhood/landmark} before a geo page ships.
  if (brief.local && brief.city) {
    const esc = (s) => String(s).replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const inCity = new RegExp(esc(brief.city), 'i').test(text);
    const localPrice = /\$\s?\d/.test(text) && inCity;
    const namedProvider = /\b(MD|DO|NP|PA-C|FNP|DNP|RN|board[- ]?certified)\b/.test(text);
    const neighborhood = (brief.neighborhoods || []).some((n) => n && new RegExp(esc(n), 'i').test(text)) || /\b(downtown|neighborhood|district|suburb|landmark|located (in|near|at))\b/i.test(text);
    hard['local-value'] = [localPrice, namedProvider, neighborhood].filter(Boolean).length >= 2;
  }

  const hardFails = Object.entries(hard).filter(([, v]) => !v).map(([k]) => k); // gate KEYS (contract); details live in `hard`/`unsourced`
  ...
  const cityHits = brief.city ? (text.match(new RegExp(brief.city.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'), 'gi')) || []).length : 0;
