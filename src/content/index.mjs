...
/** Cap programmatic plan size — mass-producing near-identical pages is the June-2026 spam-update target. */
...
export function contentPlan(cfg, { log = () => {} } = {}) {
  ...
  for (const svc of services) for (const geo of geos) {
    plan.push({ type: 'compact_keyword', targetQuery: `${svc.toLowerCase()} cost ${geo}`, url: `/${svc.toLowerCase().replace(/\s+/g, '-')}-${geo.toLowerCase().replace(/\s+/g, '-')}`, ymyl: true, local: true });
  }
  ...
  // Scaled-content guard: cap the programmatic plan. June-2026 hit mass near-identical page networks.
  { const c = capProgrammatic(plan, cfg.content?.maxProgrammaticPages ?? 200); plan = c.plan; if (c.dropped) log(`  ⚠ scaled-content guard: capped to ${plan.length} pages (dropped ${c.dropped}) — ship fewer, deeper, genuinely-unique pages, not a templated network.`); }
...
export async function contentPublish(cfg, { log = () => {}, confirm = false } = {}) {
  ...
  const maxPub = cfg.content?.maxPublishPerRun ?? 4;
  const toPublish = throttlePublish(slugs, { maxPerRun: maxPub });
  if (toPublish.length < slugs.length) log(`  ⏱ publish-velocity guard: publishing ${toPublish.length}/${slugs.length} this run (cap ${maxPub}). Burst publishing is a SpamBrain signal — the rest stay approved for the next run.`);
  const proposals = [];
  ...
  if (confirm && (res?.applied?.length || res?.pr)) {
    for (const slug of slugs) {
      ...
      appendFileSync(join(ROOT, 'reports', cfg.name, 'content-scores.ndjson'), JSON.stringify({ at: nowIso(), file: `${slug}.md`, published: true, complianceFail: !r.hard.compliance, score: r.score }) + '\n');
    }
  }
  return res;
}
...
export async function contentBatch(cfg, { log = () => {}, limit = 10 } = {}) {
  const planPath = join(ROOT, 'reports', cfg.name, 'content-plan.json');
  ...
  const siblings = []; // dedup generated service/location pages against EACH OTHER — the June-2026 spam
  for (const item of plan) {                                    // update hit "templated location-page networks" (city-swap); this catches near-dupes.
    const r = await contentDraft(cfg, item.targetQuery, { log: () => {}, priorTexts: siblings });
    if (r?.draftPath) { drafted++; if (r.score?.publishEligible) eligible++; try { siblings.push(readFileSync(r.draftPath, 'utf-8')); } catch { /* */ } }
...
async function llmDraft(tmpl, brief) {
  const reviewedMonth = new Date().toLocaleString('en-US', { month: 'long', year: 'numeric' });
  // Copywriting layer distilled from the live winning-medspa teardown (research/medspa-funnelhack-2026.md):
  // education-first authority, answer-island scaffold, specificity over superlatives, transparent pricing.
...
USE ONLY THESE FACTS (every number/claim/price must come from here — invent nothing):
...
WINNING STRUCTURE (what AI engines actually cite — each section is a self-contained answer island):
- Open with a 40-60 word ANSWER CAPSULE: treatment + city + what it does + who performs it (credential) + a starting price range. Declarative sentences <= 17 words.
- Use question-form H2s phrased exactly as patients ask ("Am I a candidate?", "How long does it last?", "Is there downtime?", "How much does ${brief.targetQuery} cost?").
- Include a transparent COST answer with a real price RANGE (only from the facts above) and put a price inside at least one FAQ answer.
- Replace vague phrasing with specifics FROM THE FACTS (e.g. "2-7 days of possible bruising", not "minimal downtime").
- Lead with the OUTCOME, then the mechanism. Close with a reassurance-first CTA ("book a free, no-pressure consultation").
End with exactly: "Medically reviewed by ${brief.author?.name}, ${brief.author?.role}. Last reviewed ${reviewedMonth}."
RULES: no "guaranteed/permanent/cure/100% safe/miracle"; no bare self-proclaimed "best/#1" in the body; no GLP-1 brand drug names; no fabricated stats/prices/reviews; specificity over superlatives; plain, varied sentences at an 8th-grade reading level.`;
  const { complete } = await import('../llm.mjs');
  let ctx = {}; try { ctx = (await import('../cost.mjs')).getCostContext() || {}; } catch { /* */ }
  return (await complete(prompt, { system: 'You are an expert med-spa SEO/AEO copywriter. Voice: education-first authority, "guided, not sold to" — specificity over superlatives, anti-overfill / natural-results, outcome-first. Output ONLY the Markdown article, grounded STRICTLY in the provided facts.', maxTokens: 2000, client: ctx.client, tag: 'content' })) || '';
}
 ...
 */
export async function contentDraft(cfg, topic, { log = () => {}, priorTexts = [] } = {}) {
  ...
  if (!brief) {
    ...
    brief = { template: tmplId, local: tmplId === 'compact_keyword', neighborhoods: cfg.neighborhoods || [], title: topic, targetQuery: topic.toLowerCase(), city: cfg.serviceAreaGeos?.[0] || '', author: { name: '', role: '' }, dataPoints: [], primarySources: [], schemaTypes, hasOwnMedia: false };
  ...
  try {
    ...
    appendFileSync(join(ROOT, 'reports', cfg.name, 'content-scores.ndjson'), JSON.stringify({ at: nowIso(), file: `${slug}.md`, score: r.score, hardPass: r.hardPass, hardFails: r.hardFails, complianceFail: !r.hard.compliance, publishEligible: r.publishEligible }) + '\n');
