...
export function rankSpas(spas = []) {
  return spas.filter((s) => (s.rating || 0) > 0)
    .map((s) => ({ ...s, _score: (s.rating || 0) * Math.log10(Math.max(0, s.review_count || 0) + 1) + (s.msd_score || 0) * 0.1 }))
...
export function cityStats(spas = []) {
  ...
  if (prices.length) {
    const tally = new Map();
    for (const p of prices) tally.set(p, (tally.get(p) || 0) + 1);
    let best = 0;
    for (const [p, n] of tally) { if (n > best) { best = n; priceMode = p; } }
  }
...
export function buildCityStats(city, state, spas) {
  ...
  const answerCapsule = `${city}, ${state} has ${s.count} med spas${s.rated ? ` averaging ${s.avgRating}★ across ${s.totalReviews.toLocaleString()} reviews` : ''}. ${ratingBands['4.5★+']} rate 4.5★ or higher${s.priceMode ? `; the most common price tier is ${s.priceMode}` : ''}.`;
...
export async function generatePages(cfg, { log = () => {}, minSpas = 5, max = 50 } = {}) {
  ...
  const dir = join(ROOT, 'reports', cfg.name, 'generated');
