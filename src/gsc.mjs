...
export async function pullGSC(cfg, { log = () => {}, range: rangeOverride } = {}) {
  ...
  try {
    ...
    log(`  GSC (${via}): ${byQuery.length} queries (${aiQueries.length} LLM-style 7+w), ${byPage.length} pages, ${opportunities.length} opportunities, ${strikingDistance.length} striking-distance, ${cannibalization.length} cannibalized${ai ? `, ${ai.length} AI-appearance rows` : ''}.`);
    return { enabled: true, via, siteUrl, range, queries: byQuery, pages: byPage, opportunities, strikingDistance, cannibalization, ai, aiQueries };
  } catch (e) {
