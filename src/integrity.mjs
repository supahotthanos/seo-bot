...
const normName = (s = '') => s.toLowerCase()
  .normalize('NFD').replace(/\p{Diacritic}/gu, '') // José → jose, so it matches a registry "Jose"
...
export function verifyReviewer(text = '', reviewers = []) {
  ...
  const hasRealCred = /\b(MD|DO|NP|PA|RN|RD|FNP|DNP|PA-C|APRN|MBBS|FAAD|PhD|NP-BC)\b/i.test(parsed.credentials || '');
  if (NON_PERSON.test(parsed.name) || (NON_PERSON.test(parsed.credentials || '') && !hasRealCred)) return { present: true, verified: false, name: parsed.name, issue: `byline names a non-person ("${parsed.name}${parsed.credentials ? ', ' + parsed.credentials : ''}") — a medical reviewer must be a named individual with a real credential` };
...
// ---- Fake-refresh guard -----------------------------------------------------------------
/** A "fake refresh" = the publish/modified date moved forward but the content didn't materially
 *  change. Re-dating without a real edit is spam-adjacent (June-2026 spam update); GENUINE freshness
 *  is a survivor signal. Returns true when before≈after (less than minChangePct visible-text change). */
export function isFakeRefresh(beforeText = '', afterText = '', { minChangePct = 5 } = {}) {
  const tok = (s) => new Set(String(s).toLowerCase().split(/\W+/).filter((w) => w.length > 1));
  const a = tok(beforeText), b = tok(afterText);
  if (!a.size && !b.size) return true; // nothing ↔ nothing = no real change
  let inter = 0; for (const w of a) if (b.has(w)) inter++;
  const changePct = Math.round((1 - inter / (a.size + b.size - inter || 1)) * 100);
