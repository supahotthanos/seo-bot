...
export function tallyConsensus(votes = [], { n = votes.length, threshold = votes.length } = {}) {
  ...
  const thr = Number.isFinite(threshold) ? Math.max(1, threshold) : Math.max(1, n || votes.length); // 0/negative/NaN threshold (malformed config) must NEVER pass below the vote-count floor
  const consensus = votes.length > 0 && safeCount >= 1 && safeCount >= thr;
  const RISK = { low: 0, medium: 1, high: 2 };
  const worst = ['low', 'medium', 'high'][votes.reduce((acc, v) => {
    const lvl = RISK[v?.risk] ?? 0;
...
export async function verifierConsensus(proposal, cfg, { n = 3 } = {}) {
  ...
  const threshold = Math.max(n, cfg.autopilot?.consensusThreshold ?? n); // floor: threshold can never be < n (unanimous or stricter)
  const votes = [];
  for (let i = 0; i < n; i++) {
