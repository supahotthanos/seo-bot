...
export function tightenText(s, max) {
  ...
  if (!(max > 0)) return clean; // NaN/0/negative max (e.g. metaMax null in config) → don't blank the text
...
export async function buildProposals(cfg, { log = () => {} } = {}) {
  ...
  for (const page of r.pages) {
    ...
    const p = page.parsed;
    const rules = new Set(Array.isArray(page.findings) ? page.findings.map((f) => f.rule) : []);
    ...
    if (rules.has('meta-description') && p.metaLen > cfg.audit.metaMax) {
      ...
      if (useLLM) {
        ...
        if (out && out.length <= cfg.audit.metaMax) proposed = out.replace(/^["']|["']$/g, '');
      }
...
export function saveProposals(result) {
  // EV-rank on a shallow copy so the caller's result.proposals reference is not mutated;
  // rankProposals annotates .priority in-place on the objects (shared refs) then returns
  // a sorted copy — we reassign result.proposals so downstream callers see the ranked order.
  result.proposals = rankProposals(result.proposals.slice()); // EV-rank (severity × demand × CTR-uplift ÷ effort) so the highest-leverage fixes lead
  const dir = join(ROOT, 'reports', result.client);
