...
export const GOOGLE_SCOPES = {
  ...
  gmail: 'https://www.googleapis.com/auth/gmail.send', // outreach agent — SEND-only, never read
};
...
/** Link an existing stored token to another name (e.g. the shared _intake account's grant
 *  becomes an auto-onboarded client's GSC credential — ONE consent covers every site the
 *  agency Gmail is granted). Re-encrypts under the same key. */
export function linkGoogleToken(fromClient, toClient) {
  const tok = loadToken(fromClient);
  if (!tok?.refresh_token) return { ok: false, error: `no stored token for "${fromClient}"` };
  saveToken(toClient, { ...tok, client: toClient, linked_from: fromClient, linked_at: new Date().toISOString() });
  return { ok: true, path: tokenPath(toClient) };
}

/** Interactive: open consent, capture the code on a loopback port, store the refresh token. */
export async function connectGoogle(client, { scopes = ['ga4', 'gsc', 'gbp'], loginHint = process.env.SEO_BOT_GOOGLE_LOGIN_HINT || null, log = () => {} } = {}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, '127.0.0.1', () => {
      const port = server.address().port;
      const params = { client_id: id, redirect_uri: `http://127.0.0.1:${port}`, response_type: 'code', scope: scopeStr, access_type: 'offline', prompt: 'consent', state, code_challenge: codeChallenge, code_challenge_method: 'S256' };
      if (loginHint) params.login_hint = loginHint; // optional (SEO_BOT_GOOGLE_LOGIN_HINT or caller-passed)
      const authUrl = `${AUTH_URL}?${new URLSearchParams(params)}`;
