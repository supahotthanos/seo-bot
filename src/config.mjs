...
// Self-contained root = the seo-bot folder itself (one level up from src/).
// All of the bot's own state (reports, data, scripts, research, sales-playbook)
// lives under here, so the folder is portable and name-independent.
...
const DEFAULTS = {
  ...
  reviewers: [],
  // notify: the Slack lanes (src/notify.mjs + src/escalate.mjs). Transports: botToken (xoxb-,
  // env SLACK_BOT_TOKEN — preferred, posts to named channels) or slackWebhook (env
  // SEO_BOT_SLACK_WEBHOOK — single-channel fallback). channels.approvals gets the queue mirror
  // (deep links …/approvals?focus=<taskId>); channels.csuite gets BIG issues (run failures,
  // guardrail trips, dead lanes — 24h dedupe). tiers picks what counts as "needs you now";
  // maxShotsPerPush caps before/after captures per dashboard push.
  notify: { slackWebhook: null, botToken: null, channels: { approvals: null, csuite: null }, dashboardUrl: null, tiers: ['red'], maxItems: 6, maxShotsPerPush: 12 },
...
const NON_CLIENT_CONFIGS = new Set(['source-trust', 'google-oauth']);
