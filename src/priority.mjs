 ...
 *  Returns 0 (sorts last) for a null/non-object proposal — fail-closed on corrupt input. */
export function scoreProposal(p) {
  if (p == null || typeof p !== 'object') return 0;
  const sev = SEVERITY_WEIGHT[p.severity] ?? 1;
  const impr = Number.isFinite(p.gsc?.impressions) && p.gsc.impressions >= 0 ? p.gsc.impressions : 0; // corrupt/Infinity/negative GSC → treat as no data
 ...
 *  Null/non-object elements are silently dropped — fail-closed on corrupt input. */
export function rankProposals(proposals = []) {
  const valid = Array.isArray(proposals)
    ? proposals.filter((p) => p != null && typeof p === 'object')
    : [];
  for (const p of valid) p.priority = scoreProposal(p);
  return valid.sort(
