...
export function pageDoc(pg, cfg) {
  ...
  $('a[href]').each((_, el) => { // reuse the SAME $ — a node from one cheerio.load is invalid in another
 ...
 */
export function buildApplyItem(srcDoc, item, cfg) {
  const anchor = item.anchor;
  const href = item.targetUrl;
  const escAnchor = anchor.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const reAnchor = new RegExp(escAnchor, 'i');
  // already-linked? the anchor phrase already sits inside an <a>…</a> in this paragraph.
  // Use a cheerio DOM query so inline elements inside the <a> (e.g. <em>, <strong>, <span>)
  // are handled correctly — the brittle [^<]* regex would miss those and produce nested <a>.
  const isAlreadyLinked = (pHtml) => {
    const $p = cheerio.load(pHtml);
    return $p('a').toArray().some((el) => reAnchor.test($p(el).text()));
  };
  // Pick the first paragraph that contains the anchor phrase verbatim and isn't already linking it.
  let chosen = null;
  for (const p of srcDoc.paragraphs) {
    if (reAnchor.test(p.text) && !isAlreadyLinked(p.html)) { chosen = p; break; }
  }
  ...
  if (!chosen) {
    // Anchor phrase not present verbatim (or already linked) — never invent prose; advisory placement.
    return { ...base, autoApplicable: false, advisory: `place a contextual link to ${href} using anchor "${anchor}" in a relevant paragraph` };
  }
  // find/replace run against the RAW paragraph HTML (apply/nextjs does find/replace on source HTML,
  // so a stripped-text find would never match). href is attribute-encoded to avoid broken/injected attrs.
  const safeHref = String(href).replace(/&/g, '%26').replace(/"/g, '%22').replace(/</g, '%3C').replace(/>/g, '%3E');
  const find = chosen.html;
  const replace = find.replace(reAnchor, (m) => `<a href="${safeHref}">${m}</a>`);
...
export async function sculptLinks(pages, pr, { cfg, gscUrls = [], logUrls = [], sitemapUrls = [], log = () => {} } = {}) {
  ...
  const placeLink = async (targetUrl, reason) => {
    ...
    if (!srcDoc) return null; // source pruned from the doc set (e.g. a hub not in ok pages) — skip, don't crash
