...
export function gateVerdict({ score = 0, bySeverity = {} } = {}, g = {}, baseline = null) {
  ...
  if (baseline) {
    const baseCrit = Number.isFinite(baseline.critical) ? baseline.critical : 0;
    if (crit > baseCrit) fails.push(`criticals regressed ${baseCrit}→${crit} vs baseline`);
    if (Number.isFinite(baseline.score) && score < baseline.score - maxScoreDrop)
      fails.push(`score regressed ${baseline.score}→${score} (> ${maxScoreDrop} drop)`);
  }
