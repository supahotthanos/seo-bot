...
export async function runLoop(cfg, { log = () => {}, apply = false, confirm = false, skipMeasure = false } = {}) {
  ...
  // Tag proposals with any GSC opportunity for the matching page (prioritization).
  if (gsc.enabled) {
    const oppByPage = new Map(gsc.opportunities.map((o) => [o.page.replace(/\/$/, ''), o]));
    for (const p of result.proposals) { const o = oppByPage.get(p.page.replace(/\/$/, '')); if (o) p.gsc = o; }
  }
