...
export async function applyNextjs(cfg, data, { log = () => {}, confirm = false } = {}) {
  ...
  for (const p of patches) {
    if (p.fileWrite) { previews.push({ p, kind: 'write', ok: true, path: p.fileWrite.path }); continue; }
  ...
  const stamp = data.ranAt.replace(/[:.]/g, '-');
  ...
  try {
    ...
    if (has('gh')) {
      try {
        ...
        pr = execFileSync('gh', ['pr', 'create', '--fill', '--base', baseBranch, '--head', branch], { cwd: repoPath, encoding: 'utf-8' }).trim();
