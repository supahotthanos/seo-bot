...
export function nonInferiorityProportion(cConv, cN, vConv, vN, { marginPct = 0.1, alpha = 0.05, biggerIsBetter = true } = {}) {
  if (cN <= 0 || vN <= 0) return { ok: false, reason: 'empty sample', breached: false, nonInferior: false };
...
export function nonInferiorityValue(control, variant, { marginPct = 0.1, biggerIsBetter = true, label = 'metric' } = {}) {
  if (control == null || variant == null) return { ok: false, breached: false, reason: `${label}: no reading` };
  if (control === 0) return { ok: true, control, variant, relChange: 0, margin: marginPct, breached: false, reason: `${label}: control baseline 0 — skipped` };
...
export function decide(input = {}, opts = {}) {
  ...
  let srm = null;
  if (input.split && input.split.length === 2) {
  ...
  if (input.conversion && input.conversion.cN > 0 && input.conversion.vN > 0) {
    ...
    checks.conversion = r; if (r.breached) breaches.push('GA4 conversion rate — ' + r.reason); else if (!r.nonInferior) uncertified.push('GA4 conversion rate');
  }
  ...
  if (input.citation && input.citation.cN > 0 && input.citation.vN > 0) {
    ...
    checks.citation = r; if (r.breached) breaches.push('AI-citation rate — ' + r.reason); else if (!r.nonInferior) uncertified.push('AI-citation rate');
  }
  ...
  if (input.clicks && input.clicks.control != null && input.clicks.variant != null) {
    ...
    checks.clicks = r; if (r.breached) breaches.push(r.reason);
  }
  ...
  if (input.cwv && input.cwv.control && input.cwv.variant) {
    ...
    for (const k of ['lcp', 'inp', 'cls']) {
      ...
      checks.cwv[k] = r; if (r.breached) breaches.push(r.reason);
    }
  }
  ...
  if (!anyData) return { decision: 'insufficient', breaches: [], uncertified, srm, checks, reason: 'no guardrail metrics supplied — cannot certify safety (fail-closed: do not auto-promote).' };
  ...
  return { decision: 'keep', breaches: [], uncertified, srm, checks, reason: 'all guardrails non-inferior' + (srm ? ' + split healthy' : '') + (uncertified.length ? ` (${uncertified.length} uncertified, allowed)` : '') + ' — safe to keep/promote.' };
}
