...
export function counterfactualImpact(series, opts = {}) {
  ...
  // d.o.f.-rescaled, centered residual pool to draw fresh noise from (keeps the empirical shape).
  const resid = sigma > 0 && ssr > 0
    ? baseResiduals.map((e) => (e - rMean) * (sigma / Math.sqrt(ssr / nBase)))
    : baseResiduals.map(() => 0);
