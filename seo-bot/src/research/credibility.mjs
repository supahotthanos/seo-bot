...
export function classifySource(sourceKeyOrUrl) {
  ...
  if (!rec) {
    ...
    rec = TRUST.sources.find(s => s.kind === 'domain' && host.includes(s.key.toLowerCase()));
  }
...
export function detectRedFlags(text = '', { isVendor = false, fundingDisclosed = false } = {}) {
  ...
  if (/\bstudy|data|analysis\b/.test(t) && !/\bmethod|sample|n\s*=|how we\b/.test(t)) flags.push('noMethodology');
  return flags;
}
...
export function dedupeIndependent(corroborators = []) {
  ...
  for (const c of corroborators) {
    const root = (c.rootKey || c.studyId || c.vendor || c.author || c.rootUrl || c.url || JSON.stringify(c)).toString().toLowerCase();
...
export function scoreClaim(claim) {
  ...
  const flags = new Set([...(claim.redFlags || []), ...srcFlags, ...detectRedFlags(claim.text || '', { isVendor: claim.isVendor || src.biasFlags?.includes('vendorFunded'), fundingDisclosed: claim.fundingDisclosed })]);
