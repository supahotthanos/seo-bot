...
export function draftSentences(draft = '') {
  ...
  // Keep the original text for exact find/replace; split on sentence terminators.
  const out = [];
  const re = /[^.!?]+[.!?]+/g; let m;
  while ((m = re.exec(body))) { const s = m[0].trim(); if (s.split(/\s+/).length >= 6) out.push(s); }
  return out;
}
...
function addsNumbers(original, rewrite) {
  ...
  return numbersIn(rewrite).some((n) => !before.has(n) && Number(n) > 9);
}
