...
export function parseDraftSignals(draft = '') {
  ...
  const hasSchema = /application\/ld\+json/i.test(text) || /"@type"\s*:/.test(text);
...
export function scoreContent(draft, terms, { cfg, corpus, primaryTerm } = {}) {
  ...
  const anchorCovered = sig.anchorTexts.length ? Math.min(1, sig.anchorTokens.length / Math.max(1, sig.anchorTexts.length)) : 0;
  ...
  const score = Math.round((coverage * 55 + structure * 25 + placement * 20));
