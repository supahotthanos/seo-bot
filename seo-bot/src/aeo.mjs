...
/** Split a paragraph into sentences (lightweight; good enough for word-count gating). */
function sentences(text) {
  return clean(text).split(/(?<=[.!?])\s+(?=[A-Z0-9])/).map((s) => s.trim()).filter(Boolean);
}
...
export function auditAeo(parsedPage = {}) {
  ...
  return {
    url: p.url,
    capsule: { present: !!answer, words: answerWords, inWindow: hasCapsule },
