...
export function eventsStorePath(client) {
  return join(ROOT, 'seo-bot', 'reports', client, 'access-events.ndjson');
}
...
export async function ingestLogs(cfg, { source, format = 'auto', append = true, botsOnly = false, log = () => {} } = {}) {
  ...
  mkdirSync(join(ROOT, 'seo-bot', 'reports', cfg.name), { recursive: true });
 ...
 */
export async function* readEvents(client, { storePath } = {}) {
  ...
  for await (const line of rl) {
    const s = line.trim();
    if (!s) continue;
    try { yield JSON.parse(s); } catch { /* skip a corrupt line */ }
  }
}
