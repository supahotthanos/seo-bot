...
export async function ga4Report(cfg, { log = () => {}, days = 28 } = {}) {
  ...
  try {
    ...
    try {
      ...
      if (ar.ok) aiReferrals = ((await ar.json()).rows || []).map((row) => ({ source: row.dimensionValues[0].value, sessions: Number(row.metricValues[0].value || 0), conversions: Number(row.metricValues[1].value || 0) }));
    } catch { /* best-effort */ }
    ...
    try {
      ...
      if (er.ok) eventBreakdown = ((await er.json()).rows || []).map((row) => ({ event: row.dimensionValues[0].value, count: Number(row.metricValues[0].value || 0), keyEvents: Number(row.metricValues[1]?.value || 0) })).sort((a, b) => b.keyEvents - a.keyEvents);
    } catch { /* best-effort */ }
