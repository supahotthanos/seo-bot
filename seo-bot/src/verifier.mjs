...
export function verifyBot(cfg, { log = () => {} } = {}) {
  ...
  const aiTrend = existsSync(join(ROOT, 'seo-bot', 'reports', cfg.name, 'ai-visibility')) || existsSync(join(ROOT, 'data', 'ai-visibility', 'trend.csv'));
  ...
  let av = aiTrend ? 6 : 0;
  if (aiSess > 0) av += Math.min(14, 6 + Math.floor(aiSess / 20));
