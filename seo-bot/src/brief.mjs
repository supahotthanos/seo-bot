...
function readLedger(client, hours) {
  const f = join(ROOT, 'seo-bot', 'reports', client, 'change-ledger.ndjson');
...
function readJson(client, file) {
  const f = join(ROOT, 'seo-bot', 'reports', client, file);
...
function readVisibility(cfg) {
  const clientDir = join(ROOT, 'seo-bot', 'reports', cfg.name, 'ai-visibility');
...
export async function portfolioBrief({ log = () => {}, hours = 24, deliver = true } = {}) {
  ...
  const dir = join(ROOT, 'seo-bot', 'reports'); mkdirSync(dir, { recursive: true });
