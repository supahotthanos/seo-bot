...
function countsFromSov(sovOrEngines, engine) {
  ...
  if (engine) {
    ...
    return { mentions: Math.round((e.visibility.pct / 100) * e.answered), answered: e.answered };
  }
  // pooled across engines
  let mentions = 0, answered = 0;
  for (const e of Object.values(engines || {})) { answered += e.answered; mentions += Math.round((e.visibility.pct / 100) * e.answered); }
