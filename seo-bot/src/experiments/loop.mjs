...
import { fieldOf, classifyProposal } from '../edge/cloaking-guard.mjs';
...
// Variant fields the edge layer can serve safely (head-only — never body text → never cloaking).
const EDGE_EXPERIMENT_FIELDS = new Set(['title', 'description', 'meta', 'canonical', 'jsonld', 'schema', 'answer-capsule', 'capsule']);
...
export function nominateClusters(cfg, proposals = [], { minPages = 4, minImpressions = 500 } = {}) {
  ...
  for (const p of proposals) {
    if (p.autoApplicable === false && fieldOf(p.type) !== 'answer-capsule') {
      // non-auto fixes can still be experimented if edge-safe; but skip the obviously human-only ones
    }
    const field = fieldOf(p.type);
    if (!EDGE_EXPERIMENT_FIELDS.has(field)) continue;       // only head-only variants on the edge
    if (!classifyProposal(p).ok) continue;                  // never anything the cloaking guard refuses
 ...
 */
export function nominate(cfg, data, { log = () => {}, horizonDays = 28, minPages = 4, minImpressions = 500, maxNew = 3, nowMs = Date.now(), baselineWindow = null } = {}) {
...
export async function applyDecisions(cfg, decisions, { log = () => {}, confirm = false, nowMs = Date.now() } = {}) {
  ...
  for (const d of decisions) {
    if (d.action === 'promote') {
      ...
      plan.setStatusCalls.push({ scope: 'experiment', id: d.experimentId, status: 'promoted' });
      ...
      plan.setStatusCalls.push({ scope: 'experiment', id: d.experimentId, status: 'rolled_back' });
      ...
      plan.setStatusCalls.push({ scope: 'experiment', id: d.experimentId, status: 'inconclusive' });
    ...
    // Persist experiment state transitions now (idempotent ledger). Tasks too, when confirm.
    if (confirm) {
      for (const c of plan.setStatusCalls.filter((s) => s.scope === 'experiment')) {
        try { setExpStatus(client, c.id, c.status, { nowMs, extra: { decision: d.action } }); } catch (e) { log(`  ! could not set experiment ${c.id} → ${c.status}: ${e.message}`); }
      }
      if (tasksMod) for (const c of plan.setStatusCalls.filter((s) => s.scope === 'task')) {
