...
export function changeClassProven(type, statsDecisions = []) {
  const forType = statsDecisions.filter((d) => d && (d.type === type || (d.taskKey || '').startsWith(`${type}:`) || classifyByPage(d) === type));
  // Unknown class → not proven (conservative): require a positive track record.
  if (!forType.length) return { proven: false, reason: 'no prior outcome data for this change-class' };
...
export function decidePolicy(proposal, cfg, { stats = [], signals = {} } = {}) {
  ...
  if ((SEVERITY_RANK[proposal.severity] ?? 9) < (SEVERITY_RANK[maxSev] ?? 3)) {
    blockers.push(`severity "${proposal.severity}" exceeds autoApplyMaxSeverity "${maxSev}"`);
  }

  // 3) Legal-sensitive med-spa surface (reviews/before-after/GLP-1/health claims).
  const isMedspa = cfg.vertical === 'medspa';
  const bodyToScan = `${proposal.type} ${proposal.current ?? ''} ${proposal.proposed ?? ''}`;
  ...
  const eligible = proposal.autoApplicable === true && (aggressive || DETERMINISTIC_FIX_TYPES.has(proposal.type));
  if (!eligible) {
    blockers.push(proposal.autoApplicable ? `change-class "${proposal.type}" is not a low-risk deterministic clamp (set autopilot.mode:"aggressive" to widen)` : 'not auto-applicable (needs authoring/review)');
    return { action: 'queue', risk: risk.tier, reasons, blockers };
  }
  reasons.push(aggressive ? `aggressive: ${proposal.type} (auto-applicable) → verifier consensus gates it` : `deterministic ${proposal.type} clamp`);
...
export async function loadStatsDecisions(client) {
  ...
  const f = join(ROOT, 'seo-bot', 'reports', client, 'decisions.ndjson');
