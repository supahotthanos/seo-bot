...
// Resolve paths the same way the modules do (src/config.mjs ROOT/CONFIG_DIR) — the old
// join(__dirname,'..','..','seo-bot',…) assumed the checkout directory is literally named
// "seo-bot" and silently wrote the fixture OUTSIDE the repo in worktrees/renamed clones.
const { ROOT: BOT_ROOT, CONFIG_DIR } = await import('../src/config.mjs');
const CLIENT = '_inttest';
const CFG_FILE = join(CONFIG_DIR, `${CLIENT}.json`);
const REP = join(BOT_ROOT, 'reports', CLIENT);
